...
if (isset($_POST['srch_mode']) || isset($_GET['srch_mode'])) {
	$dataArr = GetDataFromCOND("gen_vehicle_transmission", $cond.' order by iRank');
}
				...
				if($execute_query)
				{
					if(!empty($dataArr))
					{
						foreach ($dataArr as $i => $row)
						{
							...
							$x_name = $row->vName;
							$x_stat = $row->cStatus;
							$x_status_str = GetStatusImageString('VEHICLE_TRAN', $x_stat, $x_id, false);

...
else if($mode=='I')
{
	...
	// $txtdfrom = ConvertFromDMYToYMD(db_input($_POST['txtdfrom']));
	// $txtdto = ConvertFromDMYToYMD(db_input($_POST['txtdto']));
	$txtbasedist = db_input($_POST['txtbasedist']);
	$txtbasehrs = db_input($_POST['txtbasehrs']);
	$txtbasefare = db_input($_POST['txtbasefare']);
	$txtaddperkm = db_input($_POST['txtaddperkm']);
	$txtaddperhr = db_input($_POST['txtaddperhr']);
	$txtwatchrgperhr = db_input($_POST['txtwatchrgperhr']);
	$txtngtcharges = db_input($_POST['txtngtcharges']);
	$txtngtaddperkm = db_input($_POST['txtngtaddperkm']);
	$txtngtaddperhr = db_input($_POST['txtngtaddperhr']);
	$txtngtwatchrgperhr = db_input($_POST['txtngtwatchrgperhr']);
	$txtdrvchrgfrom = db_input($_POST['txtdrvchrgfrom']);
	$txtdrvchrgto = db_input($_POST['txtdrvchrgto']);
	$txtdrvchrgs = db_input($_POST['txtdrvchrgs']);
	$rdstatus = db_input($_POST['rdstatus']);
	$cmbvID=db_input($_POST['cmbvID']);
	...
	// {
		...
		$q = "INSERT INTO tariff (iTariffID, iVTypeID, iVCatID,iVehicleID, dFrom, dTo, fBaseDistanceInKM, fBaseHours, fBaseFare, 
				...
				VALUES ('$txtid', '$cmbtype', '$cmbcat','$cmbvID', NULL, NULL, '$txtbasedist', '$txtbasehrs', '$txtbasefare', 
...
else if($mode=='E')
{
	...
	$cmbvID=db_output($dataArr[0]->iVehicleID);
...
else if($mode=='U')
{
	...
	// if(strtotime(str_replace("'",'',$txtdto))>strtotime(str_replace("'",'',$txtdfrom)) && strtotime(str_replace("'",'',$txtdrvchrgto))>strtotime(str_replace("'",'',$txtdrvchrgfrom)))
	// {
		$q = "UPDATE tariff SET iVTypeID='$cmbtype',iVehicleID='$cmbvID', iVCatID='$cmbcat', dFrom=NULL, dTo=NULL, fBaseDistanceInKM='$txtbasedist', fBaseHours='$txtbasehrs', fBaseFare='$txtbasefare', 

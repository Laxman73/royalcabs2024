...
if(!empty($txtkeyword))
{
	$cond .= " and (vTitle LIKE '%".$txtkeyword."%')";
	$execute_query = true;
}
...
if (isset($_POST['srch_mode']) || isset($_GET['srch_mode'])) {
	$dataArr = GetDataFromCOND("slider", $cond.' order by iRank');
}
        ...
        <div class="app-inner-layout">
          ...
          <div class="card mb-3">
            <div class="card-header-tab card-header">
              <div class="card-header-title font-size-lg text-capitalize font-weight-normal"> <i class="header-icon pe-7s-edit mr-3 text-muted opacity-6"> </i><?php echo $PAGE_TITLE2; ?> </div>
				...
				if($execute_query)
				{
					if(!empty($dataArr))
					{
						foreach ($dataArr as $i => $row)
						{
							$x_id = $row->iSliderID;
							$x_title = $row->vTitle;
							$x_timebound = $YES_ARR[$row->cTimeBound];
							$x_stat = $row->cStatus;
							$x_status_str = GetStatusImageString('SLIDER', $x_stat, $x_id, false);
							$url = $edit_url.'?mode=E&id='.$x_id;
							?>
						  <tr>
							<td><?php echo $i+1; ?></td>
							<td><a href="<?php echo $url; ?>"><?php echo $x_title; ?></a></td>
							<td style="text-align:center;"><?php echo $x_timebound; ?></td>

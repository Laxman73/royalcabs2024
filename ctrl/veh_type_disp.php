...
//if (isset($_POST['srch_mode']) || isset($_GET['srch_mode'])) {
	$dataArr = GetDataFromCOND("gen_vehicle_type", $cond.' order by iRank');
//}
				...
				if($execute_query)
				{
					if(!empty($dataArr))
					{
						foreach ($dataArr as $i => $row)
						{
							...
							$x_seats = $row->vSeats;
							$x_stat = $row->cStatus;
							$x_status_str = GetStatusImageString('VEHICLE_TYPE', $x_stat, $x_id, true);
							$url = $edit_url.'?mode=E&id='.$x_id;
							?>
						  <tr>
							<td><?php echo $i+1; ?></td>
							<td><a href="<?php echo $url; ?>"><?php echo $x_name; ?></a></td>
							<!-- <td><?php echo $x_seats; ?></td> -->

...
if (isset($_POST['srch_mode']) || isset($_GET['srch_mode'])) {
	$dataArr = GetDataFromCOND("sys_settings", $cond.' order by iSettingID');
}
              ...
              <table style="width: 100%;" id="stateTable" class="table table-hover table-striped table-bordered">
				...
				if($execute_query)
				{
					if(!empty($dataArr))
					{
						...
						{
							$x_id = $row->iSettingID;
							$x_type = $SYS_SET_TYPE_ARR[$row->cType];
							$x_code = $row->vCode;
							$x_data = $SYS_SET_DATA_ARR[$row->cData];
							$x_value = $row->vValue;
							$x_stat = $row->cStatus;
							$x_status_str = GetStatusImageString('SYS_SET', $x_stat, $x_id, false);
						  ...
						  <tr>
							...
							<td><a href="<?php echo $url; ?>"><?php echo $x_code; ?></a></td>
							<td><a href="<?php echo $url; ?>"><?php echo $x_data; ?></a></td>
							<td><a href="<?php echo $url; ?>"><?php echo $x_value; ?></a></td>

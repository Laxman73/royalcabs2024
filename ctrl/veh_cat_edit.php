...
$PAGE_TITLE2 = 'Vehicle Category';
$PAGE_TITLE .= $PAGE_TITLE2;

$disp_url = 'veh_cat_disp.php';
$edit_url = 'veh_cat_edit.php';
...
else if($mode=='I')
{
	...
	LockTable('gen_vehicle_category');
	$txtid = NextID('iVCatID', 'gen_vehicle_category');
	$q = "INSERT INTO gen_vehicle_category (iVCatID, vName, vUrlName, iRank, cStatus) 
			VALUES ($txtid, $txtname, $txturlname, $txtid, $rdstatus)";
	$r = sql_query($q, "veh_cat_edit.E.38");
	UnLockTable();

	$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Category Details Successfully Inserted";
}
else if($mode=='E')
{
	$dataArr = GetDataFromID("gen_vehicle_category", "iVCatID", $txtid);
	
	$txtid = $dataArr[0]->iVCatID;
...
else if($mode=='U')
{
	...
	$q = "UPDATE gen_vehicle_category SET vName=$txtname, vUrlName=$txturlname, cStatus=$rdstatus WHERE iVCatID=$txtid ";
	$r = sql_query($q, "veh_cat_edit.E.68");
	$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Category Details Successfully Updated";
}
elseif($mode=='D')
{
	$txtid = $_GET['id'];
	...
	$chk_arr = array();
	//$chk_arr['Appointments'] = GetXFromYID('select count(*) from  where iVCatID='.$txtid.' and cStatus!="X"');
	$chk = array_sum($chk_arr);
	
	if(!$chk)
	{
		$q3 = "UPDATE gen_vehicle_category SET cStatus='X' WHERE iVCatID=$txtid";
		$r3 = sql_query($q3, 'veh_cat_edit.D.87');
		
		$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Category Details Successfully Deleted";
		$loc_str = $disp_url;
	}
	else $_SESSION[PROJ_SESSION_ID]->alert_info = "Vehicle Category Details Could Not Be Deleted Because of Existing ".(CHK_ARR2Str($chk_arr))." Dependencies";
                ...
                <div class="card-body">
                  <form class="" name="stateForm" id="stateForm" method="post" action="<?php echo $edit_url; ?>" enctype="multipart/form-data">
                    ...
                    <div class="col-md-12">
                      ...
                      <?php 
				if($mode=='E')
					echo '<button type="button" onclick="ConfirmDelete(\'Vehicle Category\',\''.$edit_url.'?mode=D&id='.$txtid.'\');" class="mt-2 btn btn-danger">Delete</button>';
				?>
                    </div>
                  </form>
                </div>

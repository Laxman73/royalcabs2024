...
else if($mode=='I')
{
	...
	LockTable('gen_vehicle_fuel');
	$txtid = NextID('iVFuelID', 'gen_vehicle_fuel');
	$q = "INSERT INTO gen_vehicle_fuel (iVFuelID, vName, vUrlName, iRank, cStatus) 
			VALUES ($txtid, $txtname, $txturlname, $txtid, $rdstatus)";
	$r = sql_query($q, "veh_fuel_edit.E.38");
	UnLockTable();

	$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Fuel Details Successfully Inserted";
}
else if($mode=='E')
{
	$dataArr = GetDataFromID("gen_vehicle_fuel", "iVFuelID", $txtid);
	
	$txtid = $dataArr[0]->iVFuelID;
...
else if($mode=='U')
{
	...
	$q = "UPDATE gen_vehicle_fuel SET vName=$txtname, vUrlName=$txturlname, cStatus=$rdstatus WHERE iVFuelID=$txtid ";
	$r = sql_query($q, "veh_fuel_edit.E.68");
	$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Fuel Details Successfully Updated";
}
elseif($mode=='D')
{
	$txtid = $_GET['id'];
	...
	$chk_arr = array();
	//$chk_arr['Appointments'] = GetXFromYID('select count(*) from  where iVFuelID='.$txtid.' and cStatus!="X"');
	$chk = array_sum($chk_arr);
	
	if(!$chk)
	{
		$q3 = "UPDATE gen_vehicle_fuel SET cStatus='X' WHERE iVFuelID=$txtid";
		$r3 = sql_query($q3, 'veh_fuel_edit.D.87');
		
		$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Fuel Details Successfully Deleted";
		$loc_str = $disp_url;
	}
	else $_SESSION[PROJ_SESSION_ID]->alert_info = "Vehicle Fuel Details Could Not Be Deleted Because of Existing ".(CHK_ARR2Str($chk_arr))." Dependencies";
                ...
                <div class="card-body">
                  <form class="" name="stateForm" id="stateForm" method="post" action="<?php echo $edit_url; ?>" enctype="multipart/form-data">
                    ...
                    <div class="col-md-12">
                      ...
                      <?php 
				if($mode=='E')
					echo '<button type="button" onclick="ConfirmDelete(\'Vehicle Fuel\',\''.$edit_url.'?mode=D&id='.$txtid.'\');" class="mt-2 btn btn-danger">Delete</button>';
				?>
                    </div>
                  </form>
                </div>

...
if (isset($_POST['srch_mode']) || isset($_GET['srch_mode'])) {
	$dataArr = GetDataFromCOND("state", $cond.' order by iCountryID, iRank');
}
        ...
        <div class="app-inner-layout">
          ...
          <div class="card mb-3">
            <div class="card-header-tab card-header">
              <div class="card-header-title font-size-lg text-capitalize font-weight-normal"> <i class="header-icon pe-7s-note2 mr-3 text-muted opacity-6"> </i><?php echo $PAGE_TITLE2; ?> </div>
				...
				if($execute_query)
				{
					if(!empty($dataArr))
					{
						...
						foreach ($dataArr as $i => $row)
						{
							...
							$x_name = $row->vName;
							$x_stat = $row->cStatus;
							$x_status_str = GetStatusImageString('STATE', $x_stat, $x_id, false);
							$url = $edit_url.'?mode=E&id='.$x_id;

							if (!isset($group_by_country[$row->iCountryID])) {
								$group_by_country[$row->iCountryID] = !empty($COUNTRY_ARR[$row->iCountryID])?$COUNTRY_ARR[$row->iCountryID]:'- Na -';
								?>
								<tr>
									<td colspan="3" class="text-center bg-warning"><b><?php echo $group_by_country[$row->iCountryID]; ?></b></td>
								</tr>
								<?php
							}
							?>

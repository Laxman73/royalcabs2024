...
$PAGE_TITLE2 = 'Vehicle Type';
$PAGE_TITLE .= $PAGE_TITLE2;

$disp_url = 'veh_type_disp.php';
$edit_url = 'veh_type_edit.php';
...
$DIPOSAL_ID = 3;
$CAT_ARR = GetXArrFromYID('select iVCatID, vName from gen_vehicle_category where cStatus!="X" and iVCatID!='.$DIPOSAL_ID.' order by iVCatID','3');
...
else if($mode=='I')
{
	...
	$cmbcat = db_input($_POST['cmbcat']);
	$rdstatus = db_input($_POST['rdstatus']);

	LockTable('gen_vehicle_type');
	$txtid = NextID('iVTypeID', 'gen_vehicle_type');
	$q = "INSERT INTO gen_vehicle_type (iVTypeID, iVCatID, vName, vUrlName, vSeats, iRank, cStatus) 
			VALUES ('$txtid', '$cmbcat', '$txtname', '$txturlname', '$txtseats', '$txtid', '$rdstatus')";
	$r = sql_query($q, "veh_type_edit.E.38");
	UnLockTable();

	$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Type Details Successfully Inserted";
}
else if($mode=='E')
{
	$dataArr = GetDataFromID("gen_vehicle_type", "iVTypeID", $txtid);
	
	$txtid = $dataArr[0]->iVTypeID;
	$txtname = db_output($dataArr[0]->vName);
	$txturlname = db_output($dataArr[0]->vUrlName);
	$txtseats = db_output($dataArr[0]->vSeats);
...
else if($mode=='U')
{
	...
	$q = "UPDATE gen_vehicle_type SET iVCatID=$cmbcat, vName=$txtname, vUrlName=$txturlname, vSeats=$txtseats, cStatus=$rdstatus WHERE iVTypeID=$txtid ";
	$r = sql_query($q, "veh_type_edit.E.68");
	$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Type Details Successfully Updated";
}
elseif($mode=='D')
{
	$txtid = $_GET['id'];
	...
	$chk_arr = array();
	//$chk_arr['Appointments'] = GetXFromYID('select count(*) from  where iVTypeID='.$txtid.' and cStatus!="X"');
	$chk = array_sum($chk_arr);
	
	if(!$chk)
	{
		$q3 = "UPDATE gen_vehicle_type SET cStatus='X' WHERE iVTypeID=$txtid";
		$r3 = sql_query($q3, 'veh_type_edit.D.87');
		
		$_SESSION[PROJ_SESSION_ID]->success_info = "Vehicle Type Details Successfully Deleted";
		$loc_str = $disp_url;
	}
	else $_SESSION[PROJ_SESSION_ID]->alert_info = "Vehicle Type Details Could Not Be Deleted Because of Existing ".(CHK_ARR2Str($chk_arr))." Dependencies";
                  ...
                  <form class="" name="stateForm" id="stateForm" method="post" action="<?php echo $edit_url; ?>" enctype="multipart/form-data">
                    ...
                    <div class="col-md-12">
                      <div class="form-row">
						...
						<div class="col-md-2">
							<div class="position-relative form-group">
								<label for="txtseats" class="">Seats <span class="text-danger"></span></label>
								<input name="txtseats" id="txtseats" type="text" value="<?php echo $txtseats; ?>" class="form-control">
							</div>
						</div>
						<div class="col-md-2">
							<div class="position-relative form-group">
                    ...
                    <div class="col-md-12">
                      ...
                      <?php 
				if($mode=='E')
					echo '<button type="button" onclick="ConfirmDelete(\'Vehicle Type\',\''.$edit_url.'?mode=D&id='.$txtid.'\');" class="mt-2 btn btn-danger">Delete</button>';
				?>
                    </div>
                  </form>
                </div>
...
$( document ).ready(function() {
	$("#stateForm").submit( function() {
		...
		// 	ShowError( txtseats, "Please enter URL Name");
		// 	err_arr[err] = txtseats;
		// 	err ++;
		// } else HideError(txtseats);

		if(cmbcat.val() == 0 || cmbcat.val() === '') {
...
//INSERT INTO whatrip.location (iLocationID, vName, cStatus)
//SELECT iLocID, vName, cStatus
//FROM royalholidays.gen_location;
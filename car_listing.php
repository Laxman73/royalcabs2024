...
$disp_url = 'car_listing.php';

if (URL_REWRITTING == 'ON') $disp_url = SITE_ADDRESS . 'hire-cars-in-goa.html';
else $disp_url = SITE_ADDRESS . 'car_listing.php';
...
$LOC_WISE_DISTANCE = array();

// $cartArr = (isset($_SESSION['cart'])) ? $_SESSION['cart'] : array('data' => array('location'=>array('cmbfrom' =>'','cmbto'=>'') , 'users_details' => array()));

//DFA($_SESSION);
$execute_query = false;
$cmbtime = $cmbpasses = $cmbfrom = $txtdate = $cmbto = $filter = $cartype = $carfuel = $cartran = $sort = $txtkeyword = '';
$Loc_ds = 0;
...
if (isset($_POST['srch_mode']) && $_POST['srch_mode'] == 'SUBMIT') {
	...
	$params = '&txtkeyword=' . $txtkeyword . '&cmbfrom=' . $cmbfrom . '&cmbto=' . $cmbto . '&cmbtime=' . $cmbtime . '&cmbpasses=' . $cmbpasses . '&filter=' . $filter . '&cartype=' . $cartype . '&carfuel=' . $carfuel . '&cartran=' . $cartran . '&sort=' . $sort . '&txtdate=' . $txtdate;
	...
	if (isset($_GET['cartype'])) $cartype = $_GET['cartype'];

	if (isset($_GET['carfuel'])) $carfuel = $_GET['carfuel'];
	if (isset($_GET['cartran'])) $cartran = $_GET['cartran'];
	if (isset($_GET['sort'])) $cartran = $_GET['sort'];

	$params2 = '?&txtkeyword=' . $txtkeyword . '&cmbfrom=' . $cmbfrom . '&cmbto=' . $cmbto . '&cmbtime=' . $cmbtime . '&cmbpasses=' . $cmbpasses . '&filter=' . $filter . '&cartype=' . $cartype . '&carfuel=' . $carfuel . '&cartran=' . $cartran . '&sort=' . $sort . '&txtdate=' . $txtdate;
...
$q = "SELECT v.*, gvt.vName AS TypeName, gvt.vSeats, tar.fBaseFare,tar.fBaseDistanceInKM as priceperkm,tar.fAdditionalPerKM as additional_perKM, CONCAT('~',GROUP_CONCAT(vca.iVCatID SEPARATOR '~'),'~') AS VCatIDs 
FROM vehicle AS v 
...
<meta charset="utf-8">
	...
	<title>Car Rentals | Car Hire in Goa | Royal Holidays</title>
	<meta name="description" content="Make your travel in Goa hassle-free with Royal Holidays. We offer car rentals in goa. Our convenient transportation solutions are tailored to your needs. " />
	<meta name="keywords" content="coach rental goa,  car hire goa, coach rental prices, rent coach, rent a bus, bus rental goa, goa bus rental,  royal holidays goa">
	<meta property="og:locale" content="en_US" />
	<meta property="og:type" content="website" />
	<meta property="og:title" content="Car Rentals | Car Hire in Goa | Royal Holidays" />
	<meta property="og:description" content="Make your travel in Goa hassle-free with Royal Holidays. We offer car rentals in goa. Our convenient transportation solutions are tailored to your needs. " />
	<meta property="og:image" content="https://royalcabsgoa.com/images/Royal-holidays-og-image.png">
	<meta property="og:url" content="https://royalcabsgoa.com/rent-cars-in-goa.html" />
	<meta property="og:site_name" content="Royal Holidays" />
	<meta name="twitter:card" content="summary_large_image" />
	<meta name="twitter:title" content="Car Rentals | Car Hire in Goa | Royal Holidays" />
	<meta name="twitter:description" content="Make your travel in Goa hassle-free with Royal Holidays. We offer car rentals in goa. Our convenient transportation solutions are tailored to your needs. " />
	<meta name="twitter:image" content="https://royalcabsgoa.com/images/Royal-holidays-og-image.png" />
	<link rel="canonical" href="https://royalcabsgoa.com/rent-cars-in-goa.html" />
...
<body>
	...
	<form class="" name="carList" id="carList" onsubmit="return ValidateForm();" action="" method="post">
		<input type="hidden" name="srch_mode" id="srch_mode" value="SUBMIT" />
		<input type="hidden" name="distance_in_KM" id="distance_in_KM" value="<?php echo $Loc_ds;?>" />
			...
			<div class="container">
				<div class="row no-gutters slider-text1 justify-content-start align-items-center justify-content-center">
					...
					<div class="text-center heading-section mt-170">
						<h2 class="text-white">Hire a perfect car</h2>
					</div>
					...
					<div class="abt-form">
						...
						<div class="d-flex gap-10">
							...
							<i class="person pdx-2"></i> -->
									<?php echo FillCombo($cmbpasses, 'cmbpasses', 'COMBO', '0', $PASS_ARR, '', 'from-control selectTime'); ?>
								</div>
							</div>
							...
							<div class="abt-form1 w-50 pd-10">
								...
								<div class="d-flex1 gap-10 just-center">
									...
									<div class="inputWithIcon inputWithIcon1">
										<input type="text" name="txtkeyword" onkeyup="getCars();" id="txtkeyword" value="<?php echo $txtkeyword; ?>" placeholder="Find a vehicle" class="border-0 input-1">
										<i class="search"></i>
									</div>
	...
	<script>
		...
		for (var i = 0; i < radioButtons.length; i++) {
			radioButtons[i].addEventListener("change", function() {
				// Code to be executed when a radio button changes

...
foreach ($dataArray as $i => $row) {
	...
	if ($execute_query) {
		...
		if (URL_REWRITTING == 'ON') $URL_COACHDISPLAY = 'summary.php';
		else $URL_COACHDISPLAY = SITE_ADDRESS . 'coach_display.php?id=' . $row->iVehicleID;
		...
		$html .= '<div class="d-flex">
									...
									<div class="container">';
		$html .= '<br>';
		if ($row->cRecommended == 'Y') {
			$html .= '<span class="card-tag1">Recommended</span>';
		}
		if (false) {
			$html .= '<span class="card-tag2">Special offer</span>';
		}
		$html .= '<br><br>';
		...
		$html .= '<i class="fuel-pump"></i><span></span>';
		$html .= '</div>';
		$html .= '<div class="column-res">';
		if ($row->cAirBags == 'Y') {
			$html .= '<i class="airbag"></i><span>Air bags</span>';
		}
		$html .= '</div>';
		$html .= '</div>';
		$html .= '<br>';
		...
		if (false) {
			$html .= '<s class="strike-text1">RS.&nbsp;6000</s>';
		} else $html .= '&nbsp;';
		$html .= '<br>';
		...
		$html .= '<a href="' . $URL_COACHDISPLAY . '?cmbfrom=' . $cmbfrom . '&cmbto=' . $cmbto . '&txtdate=' . $txtdate . '&cmbtime=' . $cmbtime . '&cmbpasses=' . $cmbpasses . '&vehicle_id=' . $row->iVehicleID . '"><button type="button" class="search-btn wpx-165 mt-2">view details</button></a>
									</div>
								</div>
							</div>';
	}

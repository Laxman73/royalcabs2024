...
$logged = 0;
$sess_user_type = 'ADM';
$is_sys_admin = $is_super_admin = $is_ho = $is_manager = $is_supervisor = $is_cashier = $is_menu_closed = $sess_property_id = false;
$sess_info_str = '';
...
{	
	if($_SESSION[PROJ_SESSION_ID]->log_stat == "A")
	{
		...
		{		
			...
			case $USER_LEVEL_ADMIN: { $is_ho = true; break; }
			case $USER_LEVEL_MANAGER: { $is_manager = true; break; }
			case $USER_LEVEL_SUPERVISOR: { $is_supervisor = true; break; }
			case $USER_LEVEL_CASHIER: { $is_cashier = true; break; }
		}
...
if($logged)
{
	##	DEFINED MENU ARRAYs	###################################################################################
	$MENU_ARR = array();
	$q = "select * from menu where cStatus='A' and iParentID=0 order by iRank";
	$r = sql_query($q, '');
	if(sql_num_rows($r))
	{
		$i = 0;
		while($mRow = sql_fetch_object($r))
		{
			...
			$MENU_ARR[$i] = array('TEXT'=>$mRow->vTitle, 'ICON'=>'fa fa-arrow-alt-circle-right', 'HREF'=>'javascript:;', 'URLS'=>$URLS, 'IS_SUB'=>$mRow->cHasSub);
				...
				$q1 = "select * from menu where cStatus='A' and iParentID ='$mRow->iMenuID' order by iRank";
				$r1 = sql_query($q1, '');
...
if($logged)
{
	...
	$_SESSION[PROJ_SESSION_ID]->info = $_SESSION[PROJ_SESSION_ID]->success_info = $_SESSION[PROJ_SESSION_ID]->error_info = $_SESSION[PROJ_SESSION_ID]->alert_info = '';	// */
}

if(!isset($_SESSION[PROJ_ALERT_SESSION_ID]))	// if there is no obj create it
{
	${PROJ_ALERT_SESSION_ID} = new alertdat;
	$_SESSION[PROJ_ALERT_SESSION_ID] = new alertdat;
	$_SESSION[PROJ_ALERT_SESSION_ID]->success_info = $_SESSION[PROJ_ALERT_SESSION_ID]->error_info = $_SESSION[PROJ_ALERT_SESSION_ID]->alert_info = $_SESSION[PROJ_ALERT_SESSION_ID]->warning_info = '';
	$_SESSION[PROJ_ALERT_SESSION_ID]->city_id = $_SESSION[PROJ_ALERT_SESSION_ID]->area_id = '0';
}

if(isset($_SESSION[PROJ_ALERT_SESSION_ID]->success_info) && !empty($_SESSION[PROJ_ALERT_SESSION_ID]->success_info))
	$sess_success_info = $_SESSION[PROJ_ALERT_SESSION_ID]->success_info;
if(isset($_SESSION[PROJ_ALERT_SESSION_ID]->error_info) && !empty($_SESSION[PROJ_ALERT_SESSION_ID]->error_info))
	$sess_error_info = $_SESSION[PROJ_ALERT_SESSION_ID]->error_info;
if(isset($_SESSION[PROJ_ALERT_SESSION_ID]->alert_info) && !empty($_SESSION[PROJ_ALERT_SESSION_ID]->alert_info))
	$sess_alert_info = $_SESSION[PROJ_ALERT_SESSION_ID]->alert_info;
if(isset($_SESSION[PROJ_ALERT_SESSION_ID]->warning_info) && !empty($_SESSION[PROJ_ALERT_SESSION_ID]->warning_info))
	$sess_warning_info = $_SESSION[PROJ_ALERT_SESSION_ID]->warning_info;

...
if ($response == "UNIQUE_CODE") {
	...
	if (isset($_GET["mode"]) && isset($_GET["status"]) && isset($_GET["id"])) {
		...
		$valid_modes = array('USERS', 'HOTEL', 'ROOM_TYPE', 'TARIFF', 'RM_CONTACTS', 'PROPERTY', 'BLOCKBOOKING_LIST', 'BLOCKBOOKING_HOTELROOM', 'BLOCKBOOKING_HOTELROOM_RM', 'COUNTRY', 'LOCATION', 'CURRENCYDENOM', 'LEVELS', 'CHIPTYPE', 'CHIPDENOM', 'TABLE', 'CAGE', 'VENDORUSERS', 'NL_CLUBS', 'AFUSERS', 'VILLA', 'NL_TICKETS', 'NL_TABLES', 'NL_EVENTS', 'OFFERS', 'OFFERS_DAT', 'LOCATION_LOCALITY', 'AMENITIES', 'MENU_SECTION', 'RESTAURANT_MENU_ITEMS', 'DINING_TABLES');
		if (in_array($mode, $valid_modes)) {
			if ($mode == 'USERS') {
				...
				$tbl = 'nl_tickets';
				$msg = 'Tickets';
			} elseif ($mode == 'NL_TABLES') {
				$pk_fld = 'iTableID';
				$tbl = 'nl_tables';
				$msg = 'Tables';
			} elseif ($mode == 'NL_EVENTS') {
				$pk_fld = 'iEventID';
				$tbl = 'nl_events';
				$msg = 'Event';
			} elseif ($mode == 'OFFERS') {
				$pk_fld = 'iOfferID';
				$tbl = 'nl_events_offers';
				$msg = 'Offer';
			} elseif ($mode == 'OFFERS_DAT') {
				$pk_fld = 'iOfferDatID';
				$tbl = 'nl_events_offers_dat';
				$msg = 'Offer Dat';
			} elseif ($mode == 'LOCATION_LOCALITY') {
				$pk_fld = 'iLocalityID';
				$tbl = 'loc_location';
				$msg = 'Location';
			} elseif ($mode == 'AMENITIES') {
				$pk_fld = 'iVAId';
				$tbl = 'amenities';
				$msg = 'Amenities';
			} elseif ($mode == 'MENU_SECTION') {
				$pk_fld = 'iMSectionID';
				$tbl = 'restaurant_menu_section';
				$msg = 'Menu section';
			} elseif ($mode == 'RESTAURANT_MENU_ITEMS') {
				$pk_fld = 'iMIID';
				$tbl = 'restaurant_menu_item';
				$msg = 'Menu Item';
			} elseif ($mode == 'DINING_TABLES') {
				$pk_fld = 'iDn_restauranTID';
				$tbl = 'dn_restaurant_tables';
				$msg = 'Resturant Table';
			}
	...
	if (isset($_GET["l_id"]) && !empty($_GET['l_id'])) {
		...
		$result = '2~Invalid Access Detected';
} else if ($response == 'DUPLICATE_EVENT') {
	$result = 0;
	$eventid = isset($_POST['eventid']) ? $_POST['eventid'] : 0;
	$result = duplicateEvent($eventid);
}

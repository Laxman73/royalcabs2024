<?php
function IsCandidateUniqueEntryForTheYear($id, $val, $ac_yr='')
{
	if(empty($ac_yr))
	{
		if(!empty($id)) $ac_yr = GetXFromYID('select iAYrID from adm_candidate where iCandidateID=$id');
		else $ac_yr = GetXFromYID('select iAYrID from adm_candidate order by iCandidateID desc limit 1');
		
		if(empty($ac_yr) || $ac_yr=='-1')
			$ac_yr = 0;
	}

	$ret_val = '2';
	$curr_txt = (isset($id))? GetXFromYID("select iGIMID from adm_candidate where iCandidateID=$id and iAYrID='$ac_yr' and cStatus!='X'"): "";
	
	if($val!='' && $val != $curr_txt) // no change in value, ignore
	{
		$q_str = (isset($id))? " and iCandidateID!=$id": "";
		$chk = GetXFromYID("select count(*) from adm_candidate where iAYrID='$ac_yr' and cStatus!='X' and iGIMID='$val' " . $q_str);

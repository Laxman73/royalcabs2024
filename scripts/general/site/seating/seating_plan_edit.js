//REMOVE ALREADY MAPPED MLAs FROM MLA LIST.
jQuery('.seat').each(function(){

	var mlaid=jQuery(this).attr('mlaid');
	if(mlaid>0){
		jQuery('.member-ul [mlaid="'+mlaid+'"]').remove();
		jQuery(this).tooltip();
	}
...
jQuery('#seating_plan_add').click(function(){
        var seatingPlanId = jQuery('#seating_plan_add').data('id');
	...
	if(jQuery.trim(vTitle).length>0) {
		...
		var dataString = 'seatingPlanMasterId='+seatingPlanId+'&vTitle='+vTitle+'&seatings='+JSON.stringify(seatings);
		jQuery.ajax({
			...
			success:function(data){
				...
				if(data=='approved'){
					alertify.log('Plan Already Approved.');
					return;
				}
				if(data=='error'){
					alertify.log('There Was An Error.');
					return;
				}
